 ...
 * =============================================================================
 * biomes.js
 * Desc: Biome assignment based on temperature, moisture, and height
...
/**
 * Get default biome data
 * @returns {Object} Biome data with names, colors, matrix, etc.
 */
export function getDefaultBiomes() {
  ...
  // Biome matrix: [moistureBand][temperatureBand] -> biomeId
 ...
 * @returns {boolean} True if wetland
 */
...
/**
 * Get biome ID for a cell based on moisture, temperature, height, and river presence
 * @param {number} moisture - Moisture value
 * @param {number} temperature - Temperature in Celsius
 * @param {number} height - Cell height
 * @param {boolean} hasRiver - Whether cell has a river
 * @param {Object} biomesData - Biome data object
...
function calculateMoisture(cellId, pack, grid) {
  ...
  // Calculate mean (simple average since we don't have d3.mean)
 ...
 * @param {Object} params.biomesData - Biome data (optional, will use default if not provided)
 * @returns {Uint8Array} Biome array
 */
export function assignBiomes({ pack, grid, options, biomesData: providedBiomesData = null }) {
  if (!pack || !pack.cells) {
  ...
  const biome = createTypedArray({ maxValue: 12, length: pack.cells.i.length }); // biomes array
